...
const consulAPI = {
	...
	checkParams(paramsObj) {
		...
		if(!consulAPI.checkIfFileExist(paramsObj.cert)) {
			throw new Error('File sent as certificate doesn\'t exist');
		}

		if(!consulAPI.checkIfFileExist(paramsObj.certSecret)) {
	...
	getModifiedIndex(type) {
		return got(`${consulAPI.consulUrl}/${consulAPI.certKeys[type]}`, {
			...
			let modifiedFile = fs.createWriteStream(`${fileName}.ModifyIndex`);
			modifiedFile.write(new Buffer(consulResult.body[0].ModifyIndex + '', 'binary'));
			modifiedFile.end();
